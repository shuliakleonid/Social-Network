...
import {
  ADD_POST,
  DELETE_POST,
  SAVE_USER_PHOTO, SAVE_USER_PROFILE,
...
export const buttonAddPost = (text: string) =>
...
export const setUpdateStatus = (status: string) =>
  ({ type: SET_UPDATE_USER_STATUS, status } as const);
export const savePhotoSuccess = (photos: { small: string; large: string }) =>
  ({ type: SAVE_USER_PHOTO, photos } as const);
export const saveProfile = (profile: ProfileAPIType) =>
  ({ type: SAVE_USER_PROFILE, profile } as const);

export const getUserProfile = (userId: string) => (
  dispatch: Dispatch<ActionType>
) => {
  usersAPI
    .getProfile(userId) //делаем на сервер запрос о данных
    .then((data) => {
      //делаем с данными что-то
      console.log(data);
...
export const getUserStatus = (userId: string) => (
  ...
  profileAPI
    .getStatus(userId) //делаем на сервер запрос о данных
    .then((data) => {
      //делаем с данными что-то
...
export const updateStatus = (status: string) => (
  ...
  profileAPI
    .updateStatus(status) //делаем на сервер запрос о данных
    .then((data) => {
...
export const saveProfileUser = (profile: ProfileAPIType) => async (
    dispatch: Dispatch<ActionType>
) => {
